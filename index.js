...
const server = http.createServer((req, res) =>{
    ...
    if (pathname === '/' || pathname === '/overview'){
        ...
        const htmlCards = productsObj.map(productObj => insertProductDataToHtmlCard(productObj, htmlCard)).join('');
        res.end(htmlOverView.replace('{%PRODUCT_CARDS%}', htmlCards));
    }
    //product page
    else if (pathname === '/product'){
        res.writeHead(200, {'Content-type': 'text/html'});
        const htmlProductPage = insertProductDataToHtmlCard(productsObj[query.id], htmlProduct);
